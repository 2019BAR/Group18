...
```{r}
# 整理商品資料成一筆
mix = merge(Z,A[,c(1,11)],by="cust")

P = mix %>% group_by(cut2_4_7,prod) %>% summarise(
  num = n(),
  price = price[1]/qty[1]
) %>% top_n(n = 10,wt = num)
...
dtm = dcast(P,formula = cut2_4_7~prod,value.var = "price")
row.names(dtm) = dtm$cut2_4_7
...
d3heatmap(termDocMatrix,colors = "Greens")
```
了解分群顧客有哪些特性
第2群顧客最近剛消費過
且算是本店的首次購買的顧客居多
...
A[,c(2:4,12)] %>% scale %>% as.data.frame %>%  split(.,A$cut2_4_7) %>% sapply(colMeans) %>% 
...
group_by(A, cut2_4_7) %>% summarise(
  ...
  scale_color_gradient(low="green",high="red") +
  scale_x_log10() + scale_y_log10() + 
  geom_text(aes(label = cut2_4_7 ),size=3) +
  theme_bw() + guides(size=F) +
  labs(title="Customer Segements",
       subtitle="(bubble_size:revenue_contribution; text:group_size)",
       color="Recency") +
...
group_by(mix,cut2_4_7,weeks) %>% 
  summarise(count = n()) %>%
  group_by(cut2_4_7) %>% 
  mutate(sum = sum(count),ratio = count/sum) %>% 
  ggplot(aes(x=as.factor(cut2_4_7),y = ratio,fill = weeks)) + geom_bar(stat = "identity")
